...
const COMP_ID = "capabilities-vertical";
const TOTAL_FRAMES = 4192;
const FINAL = "/mnt/documents/1325AI-Kayla-Capabilities-v2-Jessica-9x16.mp4";
...
for (let i = 0; i < CHUNKS.length; i++) {
  const c = CHUNKS[i];
  const out = `/tmp/cap-vertical-chunk-${i}.mp4`;
  chunkFiles.push(out);
  if (onlyChunk !== null && onlyChunk !== i) { console.log(`Skipping chunk ${i}`); continue; }
  if (process.env.SKIP_EXISTING && fs.existsSync(out)) { console.log(`Chunk ${i} cached`); continue; }
...
if (onlyChunk !== null) { console.log("Single-chunk mode."); process.exit(0); }

const listFile = "/tmp/cap-vertical-concat.txt";
fs.writeFileSync(listFile, chunkFiles.map((f) => `file '${f}'`).join("\n"));
const silent = "/tmp/cap-vertical-silent.mp4";
