...
const COMP_ID = "manual";
const TOTAL_FRAMES = 7200; // 240s @ 30fps
const FPS = 30;
const VO_PATH = path.resolve(PROJ, "public/audio/vo-manual.mp3");
const FINAL = "/mnt/documents/1325AI-Platform-Manual-4min.mp4";

// 6 chunks of 1200 frames (40s) each — fits well under 600s exec timeout
...
for (let i = 0; i < CHUNKS.length; i++) {
  const c = CHUNKS[i];
  const out = `/tmp/manual-chunk-${i}.mp4`;
  chunkFiles.push(out);
...
// Concat
const listFile = "/tmp/manual-concat.txt";
fs.writeFileSync(listFile, chunkFiles.map((f) => `file '${f}'`).join("\n"));
const silent = "/tmp/manual-silent.mp4";
