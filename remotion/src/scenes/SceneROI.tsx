...
// Scene 6: ROI / $12,100 savings (~29.5s = 885 frames)
    ...
    <AbsoluteFill style={{ backgroundColor: "#000814" }}>
      ...
      <AbsoluteFill style={{ flexDirection: "row", padding: "8% 8%" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 14 }}>
          <RolesHeader frame={frame} />
          {ROLES.map((r, i) => (
            <RoleCard key={i} title={r.title} cost={r.cost} frame={frame - (60 + i * 50)} />
          ))}
          <SubtotalLine frame={frame - (60 + ROLES.length * 50 + 30)} />
        </div>
        {/* RIGHT: big savings */}
        <div style={{ flex: 1.05, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", paddingLeft: 60 }}>
          <SavingsBadge frame={frame - 380} />
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
...
const RolesHeader: React.FC<{ frame: number }> = ({ frame }) => {
...
const SavingsBadge: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: playfair, fontWeight: 900, fontSize: 180, color: "#FFFFFF", lineHeight: 0.95, letterSpacing: -6 }}>
        ${num.toLocaleString()}<span style={{ fontSize: 56, color: "#FFB300" }}>+</span>
      </div>
