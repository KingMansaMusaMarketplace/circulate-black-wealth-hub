CREATE OR REPLACE FUNCTION public.award_qr_scan(p_qr_code_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Per-user cooldown: prevents the same user from farming the same QR.
  -- Closes a double-spend gap in the atomic loyalty pipeline (patent claim 5).
  SELECT MAX(scan_date) INTO v_recent_scan
  FROM public.qr_scans
  WHERE qr_code_id = p_qr_code_id
    AND customer_id = v_user_id;

  IF v_recent_scan IS NOT NULL AND v_recent_scan > (now() - v_cooldown) THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'scan_cooldown',
      'next_available_at', v_recent_scan + v_cooldown
    );
  END IF;
