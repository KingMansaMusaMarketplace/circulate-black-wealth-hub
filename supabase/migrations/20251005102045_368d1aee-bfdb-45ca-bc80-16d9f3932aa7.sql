...
-- Fix 2: Drop ALL existing policies on businesses table
...
-- Fix 3: Create new comprehensive non-recursive policies for businesses table
...
-- Fix 4: Add public SELECT policy for businesses
CREATE POLICY "Public view businesses"
ON public.businesses
FOR SELECT
TO public
USING (true);

-- Fix 5: Create secure view for public business listings (no sensitive data)
...
-- Fix 6: Update profiles policy (remove bypassable audit function)
...
-- Fix 7: Strengthen access_personal_data_secure to properly block on failure
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    -- Block access by re-raising exception
    RAISE;
END;
$$;