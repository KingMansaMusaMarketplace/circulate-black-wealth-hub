...
BEGIN
  ...
  WHERE b.listing_status = 'live'
    AND (
      ...
      OR NOT EXISTS (
        ...
        WHERE NOT COALESCE(
          coalesce(b.business_name, '') ILIKE '%' || tok || '%' OR
          coalesce(b.name, '')          ILIKE '%' || tok || '%' OR
          coalesce(b.category, '')      ILIKE '%' || tok || '%' OR
          coalesce(b.description, '')   ILIKE '%' || tok || '%' OR
          coalesce(b.city, '')          ILIKE '%' || tok || '%' OR
          coalesce(b.state, '')         ILIKE '%' || tok || '%' OR
          coalesce(b.address, '')       ILIKE '%' || tok || '%',
          false
        )
      )
    )
  ...
  ORDER BY
    -- Location-match boost: rank 0 if any token matches city/state, else 1
    (CASE
      WHEN coalesce(array_length(v_tokens, 1), 0) = 0 THEN 1
      WHEN EXISTS (
        SELECT 1 FROM unnest(v_tokens) AS tok
        WHERE coalesce(b.city, '')  ILIKE '%' || tok || '%'
           OR coalesce(b.state, '') ILIKE '%' || tok || '%'
      ) THEN 0
      ELSE 1
    END) ASC,
    -- Name/category match next (stronger signal than description-only)
    (CASE
      WHEN coalesce(array_length(v_tokens, 1), 0) = 0 THEN 1
      WHEN EXISTS (
        SELECT 1 FROM unnest(v_tokens) AS tok
        WHERE coalesce(b.business_name, '') ILIKE '%' || tok || '%'
           OR coalesce(b.name, '')          ILIKE '%' || tok || '%'
           OR coalesce(b.category, '')      ILIKE '%' || tok || '%'
      ) THEN 0
      ELSE 1
    END) ASC,
    b.is_verified DESC,
