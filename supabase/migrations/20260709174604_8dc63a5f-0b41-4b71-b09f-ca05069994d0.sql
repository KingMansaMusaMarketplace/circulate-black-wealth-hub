...
BEGIN
  -- Authorization: only admins or service_role may credit wallets
...
DECLARE
  ...
  v_transaction_id UUID;
BEGIN
  -- Authorization: only the wallet owner or admins may debit
  IF auth.uid() IS DISTINCT FROM p_user_id AND NOT public.is_admin_secure() THEN
...
REVOKE EXECUTE ON FUNCTION public.credit_wallet(uuid, numeric, text, text, uuid, text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.debit_wallet(uuid, numeric, text, text, uuid, text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.process_withdrawal(uuid) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.debit_wallet(uuid, numeric, text, text, uuid, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.process_withdrawal(uuid) TO service_role;
