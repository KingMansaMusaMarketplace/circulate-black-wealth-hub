...
// Runs as a background task so the HTTP request returns immediately and the worker keeps
// chewing through the entire backlog until empty.
...
const MAX_BATCHES = 100; // safety cap: 100 * 200 = 20K leads per invocation
...
async function runBackfill(supabase: any) {
  ...
  for (let batch = 0; batch < MAX_BATCHES; batch++) {
    ...
    console.log(`batch ${stats.batches} done`, {
  ...
  console.log("bulk-promote-backlog FINAL", {
    ...stats,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fire-and-forget background task — survives past the HTTP response.
    // @ts-ignore EdgeRuntime is provided by Supabase Edge runtime.
    EdgeRuntime.waitUntil(runBackfill(supabase));

    return new Response(
      JSON.stringify({
        status: "started",
        backlog_at_start: backlogCount ?? null,
        message: "Background flush running. Tail edge function logs for batch progress.",
      }),
