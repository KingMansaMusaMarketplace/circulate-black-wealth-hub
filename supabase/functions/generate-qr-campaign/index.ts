...
const QRCampaignRequestSchema = z.object({
  businessId: z.string().regex(uuidRegex, 'Invalid UUID format for businessId'),
  campaignType: z.string().max(100).optional().nullable(),
  currentQRData: z.record(z.unknown()).optional().nullable(),
});
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parseResult = QRCampaignRequestSchema.safeParse(rawBody);
