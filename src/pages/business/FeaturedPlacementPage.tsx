...
export default function FeaturedPlacementPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-mansablue via-mansablue/90 to-black text-zinc-100 selection:bg-mansagold/30">
      ...
      {profileLoading && (
        <Card className="mb-8 bg-mansablue/40 border-blue-300/20">
          <CardContent className="py-8 flex items-center gap-3 text-blue-100/90">
      ...
      {!profileLoading && user && !profile && (
        <Card className="mb-8 bg-mansablue/40 border-mansagold/40">
          <CardContent className="py-6 space-y-3">
            <h2 className="text-xl font-semibold text-blue-50">We couldn't find a business linked to your account</h2>
      ...
      <IOSPaymentBlocker>
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          {TIERS.map((t) => {
            ...
            return (
              <Card
                key={t.id}
                className={`cursor-pointer transition bg-mansablue/40 ${selected ? 'border-mansagold ring-2 ring-mansagold/60' : 'border-blue-300/20 hover:border-mansagold/50'}`}
                onClick={() => setTier(t.id)}
              >
                <CardHeader>
                  <Icon className="h-6 w-6 text-mansagold mb-2" />
                  <CardTitle className="flex items-center justify-between text-blue-50">
                    {t.name}
                    {selected && <Badge className="bg-mansagold text-mansablue hover:bg-mansagold">Selected</Badge>}
                  </CardTitle>
                  <CardDescription className="text-mansagold font-semibold">${t.price}/mo</CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-blue-100">{t.blurb}</p>
                </CardContent>
              </Card>
            );
          })}
        ...
        <Card className="mb-8 bg-mansablue/40 border-blue-300/20">
          <CardHeader>
            ...
            <CardDescription className="text-blue-100/90">Choose where your business gets promoted.</CardDescription>
          </CardHeader>
          <CardContent className="grid md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="cat" className="text-blue-50">Category (optional)</Label>
              <Input id="cat" className="mt-1 bg-mansablue/60 border-blue-300/25 text-blue-50 placeholder:text-blue-200/50" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. food, beauty" />
            </div>
            <div>
              <Label htmlFor="city" className="text-blue-50">City (optional)</Label>
              <Input id="city" className="mt-1 bg-mansablue/60 border-blue-300/25 text-blue-50 placeholder:text-blue-200/50" value={city} onChange={(e) => setCity(e.target.value)} placeholder="e.g. Chicago" />
            </div>
          </CardContent>
        </Card>

        <Button size="lg" onClick={checkout} disabled={loading} className="w-full md:w-auto bg-mansagold text-mansablue font-semibold hover:bg-mansagold/90">
        ...
        {active.length > 0 && (
          <div className="mt-12">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="outline" size="sm" onClick={openPortal} className="bg-mansablue/60 border-blue-300/25 text-blue-50 hover:bg-mansablue/80 hover:text-white">
            ...
            <div className="space-y-2">
              {active.map((p) => {
                ...
                return (
                  <Card key={p.id} className="bg-mansablue/40 border-blue-300/20">
                    <CardContent className="py-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-medium capitalize text-blue-50">{p.tier} placement</div>
                      ...
                      {p.status === 'active' && (
                        <div className="grid grid-cols-3 gap-3 pt-2 border-t border-blue-300/20">
                          <div>
                            <div className="text-xs text-blue-100/90">Impressions (30d)</div>
                            <div className="text-lg font-semibold text-blue-50">{s.impressions.toLocaleString()}</div>
                          </div>
                          <div>
                            <div className="text-xs text-blue-100/90">Clicks (30d)</div>
                            <div className="text-lg font-semibold text-blue-50">{s.clicks.toLocaleString()}</div>
                          </div>
                          <div>
                            <div className="text-xs text-blue-100/90">CTR</div>
                            <div className="text-lg font-semibold text-blue-50">{ctr}%</div>
                          </div>
                        </div>
                      )}
                    </CardContent>
            ...
            <p className="text-xs text-blue-100/70 mt-3">
