...
export const handleApiError = async (
  ...
  try {
    // Log error using secure handle_api_error function
    const { data, error: logError } = await supabase.rpc('handle_api_error', {
      operation_name: operationName,
      error_message: errorMessage,
      error_details: errorDetails as any
    });

    if (!logError && data) {
      // Extract reference from the structured response
      const errorRef = data.error_reference;
      if (errorRef) {
        console.error(`Error logged with reference: ${errorRef}`);
        
        const loggedError = new Error(errorMessage) as LoggedError;
        loggedError.reference = errorRef;
...
export const checkRateLimit = async (
  ...
  try {
    // Use secure check_rate_limit function
...
export const logActivity = async (
  ...
  try {
    // Use secure log_activity function
